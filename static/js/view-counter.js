...
(function(window, document) {
    ...
    var counterRequestControllers = [];
    var counterRefreshSerial = 0;
    var counterRefreshHandle = null;

    function cancelCounterRequests() {
        counterRefreshSerial += 1;
        if (counterRefreshHandle !== null) {
            if (typeof window.cancelIdleCallback === 'function') window.cancelIdleCallback(counterRefreshHandle);
            else window.clearTimeout(counterRefreshHandle);
            counterRefreshHandle = null;
        }
        counterRequestControllers.slice().forEach(function(controller) {
            try { controller.abort(); } catch (error) {}
        });
        counterRequestControllers = [];
    }
    ...
    function syncMetaDividers(root) {
    ...
    async function requestBatchCounts(ids, increment, refreshSerial) {
        ...
        if (refreshSerial !== counterRefreshSerial) return null;
        var config = counterConfig();
        var controller = typeof AbortController === 'function' ? new AbortController() : null;
        if (controller) counterRequestControllers.push(controller);
        ...
        try {
            ...
            if (controller) {
                var controllerIndex = counterRequestControllers.indexOf(controller);
                if (controllerIndex >= 0) counterRequestControllers.splice(controllerIndex, 1);
            }
        }
    }

    async function refreshPageCounters(root) {
        var refreshSerial = ++counterRefreshSerial;
        root = root && typeof root.querySelectorAll === 'function' ? root : document;
        var config = counterConfig();
        var counters = Array.prototype.slice.call(root.querySelectorAll('[data-view-count][data-view-id]'));
        if (!config.enabled || !endpointUrl()) {
            counters.forEach(hideCount);
            syncMetaDividers(root);
            return;
        }
        counters.forEach(hideCount);
        var fullArticle = root.querySelector('article.post-full');
        ...
        var visibleCounters = counters.filter(function(counter) {
            return counter !== fullCounter && (config.showOnPreview !== false || !counter.closest('[data-argon-article-preview]'));
        });
        ...
        var results = await Promise.all(chunks.map(function(chunk) {
            ...
            return requestBatchCounts(chunk, chunk.indexOf(increment) >= 0 ? increment : '', refreshSerial);
        }));
        if (refreshSerial !== counterRefreshSerial) return;
        var values = {};
        var successful = true;
    ...
    function schedulePageCounters(root) {
        if (counterRefreshHandle !== null) {
            if (typeof window.cancelIdleCallback === 'function') window.cancelIdleCallback(counterRefreshHandle);
            else window.clearTimeout(counterRefreshHandle);
        }
        var run = function() {
            counterRefreshHandle = null;
            refreshPageCounters(root);
        };
        if (typeof window.requestIdleCallback === 'function') {
            counterRefreshHandle = window.requestIdleCallback(run, {timeout: 2000});
        } else {
            counterRefreshHandle = window.setTimeout(run, 1000);
        }
    }

    document.addEventListener('argon:navigation-start', cancelCounterRequests);
