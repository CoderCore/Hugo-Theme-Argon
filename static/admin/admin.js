(function () {
    ...
    function bootstrapLogin() { discoverEndpoint().then(function () { setStatus('请输入管理员密钥登录。', 'pending'); }).catch(function (error) { setStatus('未发现可用的阅读量 Worker：' + error.message, 'error'); }); byId('admin-login-form').addEventListener('submit', function (event) { event.preventDefault(); login(); }); }
