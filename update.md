...
- 根据 `toolbar.blur` 配置决定是否启用毛玻璃；补充无 Banner 页面处理、滚动边界保护和窗口尺寸变化重新计算。
- 使用 `requestAnimationFrame` 合并滚动更新，减少高频滚动事件对页面性能的影响。
- 为 `argontheme.js` 增加版本参数，避免生产环境继续加载旧缓存。
...
- 将上下左右内边距统一为 `3px`，使徽标外框保持对称。
- 悄悄话评论不再显示“博主”徽标，避免暴露作者身份。
