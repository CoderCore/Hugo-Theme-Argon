...
6. `[-]` 评论生产化接入：已接入 GitHub 登录、登录状态、退出登录、访客开关、真实域名 CORS 结构和 Markdown；待配置真实 OAuth Secret/回调域名，并补齐编辑/删除、审核、限流和公网联调。
...
- `[x]` 不执行 GitHub 登录或评论的真实公网联调，直到用户提供 OAuth App 参数、回调域名和部署授权。
